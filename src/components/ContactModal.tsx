...
const ContactModal: React.FC<ContactModalProps> = ({ open, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div
        className="relative bg-slate-50 rounded-3xl flex flex-col justify-between items-center 
        py-6 px-4 sm:py-8 sm:px-8 md:px-12 w-full max-w-lg sm:max-w-2xl md:max-w-3xl max-h-[95vh]"
      >
        {/* Close button */}
        {!success && (
          <button
            onClick={onClose}
            className="absolute top-2 right-4 text-gray-500 hover:text-gray-800 text-2xl sm:text-3xl"
          >
            ×
          </button>
        )}
        ...
        {!success ? (
          <>
            {/* Title */}
            <div className="text-center mb-4 px-2">
              <span className="block text-gray-800 text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold font-['IBM_Plex_Sans'] tracking-wide leading-snug">
                Enter details and Book your <br className="hidden sm:block" />
              </span>
              <span className="block text-blue-900 text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold font-['IBM_Plex_Sans'] tracking-wide leading-snug">
                <span className="text-gray-800">demo with </span> Our Platform
              </span>
            </div>
            ...
            <div className="w-full flex flex-col justify-start items-center">
              {/* Row 1 */}
              <div className="px-2 sm:px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-10 md:gap-14 w-full">
                {/* Name */}
                <div className="flex flex-col gap-1">
                  <label className="text-zinc-700 text-sm sm:text-base md:text-lg font-medium font-['Nunito Sans']">
                    Enter Name
                  </label>
                  <input
                    type="text"
                    value={form.name}
                    onChange={(e) => setForm({ ...form, name: e.target.value })}
                    className="w-full px-3 py-2 sm:px-4 sm:py-2.5 rounded-lg border border-slate-300 
                      text-sm sm:text-base md:text-lg text-gray-800 font-['Circular Std'] 
            ...
            <div className="w-full flex justify-center mt-3 sm:mt-4 px-2">
              <button
                ...
                className="w-full sm:w-auto 
                  ...
                  bg-blue-900 rounded-lg text-white 
          ...
          // ✅ Success View
          <div className="flex max-w-full flex-col items-center justify-center gap-6 py-6 px-4 sm:px-6 md:px-8">
            <img
              src={GIF}
              alt="success"
              className="w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32"
            />
            <div className="w-full max-w-2xl text-center text-gray-800 text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold font-['Nunito Sans'] leading-snug">
              Demo Request Confirmed!
            </div>
            <div className="w-full text-center text-slate-500 text-sm sm:text-base md:text-lg font-normal font-['Nunito Sans'] leading-relaxed">
              Thank you for your interest. We've received your <br/>
              demo request and a member of our team will be in touch shortly to <br/>
              schedule a convenient time
            </div>
